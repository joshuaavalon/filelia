...
CREATE TRIGGER tag_fts_bu BEFORE UPDATE ON tag BEGIN
  DELETE FROM tag_fts WHERE id = old.id;
END;
CREATE TRIGGER tag_fts_bd BEFORE DELETE ON tag BEGIN
  DELETE FROM tag_fts WHERE id = old.id;
END;
...
CREATE TRIGGER project_fts_bu BEFORE UPDATE ON project BEGIN
  DELETE FROM project_fts WHERE id = old.id;
END;
CREATE TRIGGER project_fts_bd BEFORE DELETE ON project BEGIN
  DELETE FROM project_fts WHERE id = old.id;
END;
